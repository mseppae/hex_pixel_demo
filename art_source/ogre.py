# Ogre sprite sheet: 40 x 48 frames, 6 directions x 9 poses.
...
# ---------------------------------------------------------------------------
# The three hand-set views. Each pose is composed as a whole silhouette.
# ---------------------------------------------------------------------------
...
def front_right(legs_variant, pose):
    ...
    if pose in ("rest", "idle_b"):
        layers.append(Layer("skin").thick_line(10, 22, 21, 31, 4.5))        # back arm reaching across
        club, nails = club_layers(24 if pose == "idle_b" else 22, 34, 35, 10 if pose == "idle_b" else 7)
        layers += club
        details.update(nails)
        layers.append(Layer("skin").thick_line(30, 22, 25 if pose == "idle_b" else 26, 31 if pose == "idle_b" else 30, 4.5))
        layers.append(Layer("skin").ellipse(22, 31, 2.6, 2.6))
        layers.append(Layer("skin").ellipse(25 if pose == "idle_b" else 26, 30.5 if pose == "idle_b" else 29.5, 2.6, 2.6))
    elif pose == "wind_up":
    ...
    elif pose == "strike":  # club smashed down in front
    ...
    else:  # recoil: arms and club are pulled low after a hit
        layers.append(Layer("skin").thick_line(10, 22, 16, 31, 4.5))
        layers.append(Layer("skin").thick_line(30, 22, 24, 33, 4.5))
        club, nails = club_layers(24, 33, 35, 40)
        layers += club
        details.update(nails)
        layers.append(Layer("skin").ellipse(16, 31, 2.6, 2.6))
        layers.append(Layer("skin").ellipse(24, 33, 2.6, 2.6))
    return compose(layers, details)

def right(legs_variant, pose):
    layers, details = [], {}
    if pose in ("rest", "idle_b"):
        club, nails = club_layers(27, 31, 11 if pose == "idle_b" else 9, 9 if pose == "idle_b" else 6)
        layers += club
        details.update(nails)
    elif pose == "wind_up":
    ...
    details.update({(27, 15): "brow", (28, 15): "brow", (29, 15): "brow", (29, 16): "eye",
                    ...
                    (31, 20): "tusk"})
    if pose in ("rest", "idle_b"):
        layers.append(Layer("skin").thick_line(18, 22, 26 if pose == "idle_b" else 27, 31 if pose == "idle_b" else 30, 4.5))
        layers.append(Layer("skin").ellipse(26.5 if pose == "idle_b" else 27.5, 31.5 if pose == "idle_b" else 30.5, 2.8, 2.8))
    elif pose == "wind_up":
        ...
        layers.append(Layer("skin").thick_line(18, 22, 24, 32, 4.5))
        club, nails = club_layers(24, 32, 36, 39)
        layers += club
        details.update(nails)
        layers.append(Layer("skin").ellipse(24, 32, 2.8, 2.8))
    return compose(layers, details)
...
def back_right(legs_variant, pose):
    ...
    if pose in ("rest", "idle_b"):
        club, nails = club_layers(28, 31, 33, 8 if pose == "idle_b" else 5)
        layers += club
        details.update(nails)
        layers.append(Layer("skin").thick_line(30, 22, 27 if pose == "idle_b" else 28, 31 if pose == "idle_b" else 30, 4.5))
        layers.append(Layer("skin").ellipse(27 if pose == "idle_b" else 28, 31, 2.6, 2.6))
    elif pose == "wind_up":
        ...
        layers.append(Layer("skin").thick_line(30, 22, 26, 33, 4.5))
        club, nails = club_layers(26, 33, 36, 40)
        layers += club
        details.update(nails)
        layers.append(Layer("skin").ellipse(26, 33, 2.6, 2.6))
    return compose(layers, details)

def front_right_death(pose):
    """Hand-composed crouch and grounded corpse, viewed from the front-right."""
    layers, details = [], {}
    if pose == "death_start":
        layers += [Layer("skin").rect(12, 37, 18, 44).rect(10, 43, 19, 45),
                   Layer("skin").rect(22, 38, 28, 44).rect(21, 43, 31, 45),
                   Layer("skin").ellipse(18, 33, 12, 8),
                   Layer("belly").ellipse(21, 35, 7, 4),
                   Layer("fur").rect(10, 37, 29, 40),
                   Layer("skin").ellipse(14, 26, 6.5, 5.5),
                   Layer("skin").thick_line(11, 32, 7, 40, 4.5),
                   Layer("skin").thick_line(27, 32, 23, 41, 4.5)]
        club, nails = club_layers(23, 41, 35, 44)
        layers += club; details.update(nails)
        details.update({(13, 25): "brow", (15, 26): "eye", (17, 29): "mouth", (14, 30): "tusk"})
    else:
        layers += [Layer("wood").thick_line(4, 43, 19, 43, 3.0),
                   Layer("skin").ellipse(19, 40, 13, 5),
                   Layer("belly").ellipse(18, 41, 7, 3.5),
                   Layer("fur").rect(10, 41, 28, 44),
                   Layer("skin").ellipse(31, 39, 6.5, 5),
                   Layer("skin").thick_line(12, 40, 7, 44, 4.0),
                   Layer("skin").thick_line(25, 40, 30, 44, 4.0)]
        details.update({(33, 38): "brow", (34, 39): "eye", (35, 41): "mouth", (32, 42): "tusk"})
    return compose(layers, details)


def right_death(pose):
    """Hand-composed crouch and grounded corpse, viewed from the right."""
    layers, details = [], {}
    if pose == "death_start":
        layers += [Layer("skin").rect(11, 38, 17, 44).rect(9, 43, 19, 45),
                   Layer("skin").rect(21, 38, 27, 44).rect(20, 43, 30, 45),
                   Layer("skin").ellipse(17, 34, 11, 8),
                   Layer("belly").ellipse(22, 36, 5, 4),
                   Layer("fur").rect(10, 38, 28, 41),
                   Layer("skin").ellipse(26, 28, 5.5, 5).rect(26, 29, 32, 32),
                   Layer("skin").thick_line(18, 32, 22, 41, 4.5)]
        club, nails = club_layers(22, 41, 36, 44)
        layers += club; details.update(nails)
        details.update({(28, 26): "brow", (29, 28): "eye", (31, 31): "mouth", (31, 30): "tusk"})
    else:
        layers += [Layer("wood").thick_line(4, 43, 19, 43, 3.0),
                   Layer("skin").ellipse(18, 41, 14, 5),
                   Layer("belly").ellipse(22, 41, 5, 3.5),
                   Layer("fur").rect(9, 41, 27, 44),
                   Layer("skin").ellipse(32, 39, 5.5, 5).rect(31, 40, 37, 43),
                   Layer("skin").thick_line(14, 40, 9, 44, 4.0)]
        details.update({(33, 38): "brow", (34, 39): "eye", (36, 42): "mouth", (36, 41): "tusk"})
    return compose(layers, details)


def back_right_death(pose):
    """Hand-composed crouch and grounded corpse, viewed from the back-right."""
    layers, details = [], {}
    if pose == "death_start":
        layers += [Layer("skin").rect(12, 38, 18, 44).rect(10, 43, 20, 45),
                   Layer("skin").rect(22, 38, 28, 44).rect(21, 43, 31, 45),
                   Layer("skin").ellipse(19, 34, 12, 8),
                   Layer("skin").ellipse(17, 30, 8, 5),
                   Layer("fur").rect(10, 38, 29, 41),
                   Layer("skin").ellipse(23, 27, 6, 5.5),
                   Layer("skin").thick_line(28, 32, 25, 41, 4.5)]
        club, nails = club_layers(25, 41, 36, 44)
        layers += club; details.update(nails)
        details[(28, 26)] = "brow"
    else:
        layers += [Layer("wood").thick_line(4, 43, 19, 43, 3.0),
                   Layer("skin").ellipse(19, 41, 14, 5),
                   Layer("skin").ellipse(16, 39, 8, 3.5),
                   Layer("fur").rect(10, 41, 29, 44),
                   Layer("skin").ellipse(31, 39, 6, 5.5),
                   Layer("skin").thick_line(25, 40, 30, 44, 4.0)]
        details[(35, 38)] = "brow"
    return compose(layers, details)
...
POSES = [
    ("idle", "rest"), ("step_a", "rest"), ("step_b", "rest"),
    ("idle", "wind_up"), ("idle", "strike"), ("idle", "idle_b"),
    ("idle", "recoil"), ("idle", "death_start"), ("idle", "death_down"),
]
...
def build_ogre_sheet():
    ...
    for row, (legs_variant, pose) in enumerate(POSES):
        if pose.startswith("death_"):
            fr, rt, br = front_right_death(pose), right_death(pose), back_right_death(pose)
        else:
...
def validate_sheet(sheet):
    contract = json.loads((ROOT / "assets" / "ogre_sprite_contract.json").read_text())
    if sheet.mode != "RGBA" or list(sheet.size) != contract["sheet_size"]:
        raise ValueError(f"ogre sheet must be RGBA {contract['sheet_size']}")
    width, height = contract["frame_size"]
    columns, rows = contract["grid"]
    if width != FRAME_WIDTH or height != FRAME_HEIGHT or columns != 6 or rows != len(POSES):
        raise ValueError("ogre contract and generator layout disagree")
    cells = []
    for row in range(rows):
        for column in range(columns):
            cell = sheet.crop((column * width, row * height, (column + 1) * width, (row + 1) * height))
            if not any(pixel[3] for pixel in cell.get_flattened_data()):
                raise ValueError(f"empty ogre cell ({column}, {row})")
            cells.append(cell.tobytes())
    if len(set(cells)) != columns * rows:
        raise ValueError("ogre cells must be unique")
    if not any(pixel[3] == 0 for pixel in sheet.get_flattened_data()):
        raise ValueError("ogre sheet must retain transparent pixels")

if __name__ == "__main__":
    print("This is the legacy hand-drawn Ogre source. Run assemble_modern_ogre.py for assets/ogre_sheet.png.")
